...
predicted_marginal_effect = function(model, feature, data, grid.size) {
    ...
    res = effects$results
    pdp = res[which(res$.type == "pdp"),c(feature, ".value")]
    names(pdp) = c(feature, "mean")
    ice = res[which(res$.type == "ice"),]
    
    return(list("pdp" = pdp, "ice" = ice))
}
...
marginal_effect_sd_over_mean = function(model, feature, data, grid.size, method, alpha = 0.05, correction = NULL) {
    ...
    if (method %in% c("pdp_sd", "pdp_var")) {
	    ...
	    if (method == "pdp_sd") {
	    	res.pdp$sd = effects$results$.value[which(effects$results$.type == "pdp")]
	    	res.ice$sd = effects$results$.value[which(effects$results$.type == "ice")]
	    } 
	    if (method == "pdp_var") {
	    	res.pdp$sd = sqrt(1 / nrow(data) * effects$results$.value[which(effects$results$.type == "pdp")]) # we have to devide once more by n --> see formula
